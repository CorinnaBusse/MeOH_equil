...
P_ges_list = st.sidebar.multiselect(
    "Select Pressures (atm):", [1, 2, 3, 4, 5, 10, 20, 50, 100, 200, 500], default=[1, 10, 100]
)

T_min = st.sidebar.slider("Min Temperature (K)", 298, 873, 300)
T_max = st.sidebar.slider("Max Temperature (K)", 298, 873, 800)

K_model = st.sidebar.radio("Equilibrium Constant Model", ["van't Hoff", "Bartsch"], index=0)
...
for P in P_ges_list:
    ...
    for T in T_array:
        try:
            xi = bisect(Funk, 1E-9, 1 - 1E-9, args=(P, T, K_vantHoff if K_model == "van't Hoff" else K_bartsch))
            X_GGW = xi / 1 * 100
        except:
            X_GGW = np.nan
...
ax.set_title("Equilibrium Conversion vs. Temperature")
